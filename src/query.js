 ...
 *
 * @constructor Takes no arguments. Call builder functions to
 *   construct the query.
 */
    ...
    let convertWhere = function (where) {
        if (Array.isArray(where)) {
            if (typeof where[0] === 'string' && typeof where[1] === 'string') {
                return [transit.symbol(where[0]), transit.keyword(where[1])].concat(where.slice(2).map(transit.symbol));
            } else {
...
/**
 * Build a pull expression for use in a datomic query.
 *
...
/**
 * Return a count expression, for use in QueryBuilder.find.
